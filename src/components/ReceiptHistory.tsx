...
const parseCredentialPayload = (token: string | null) => {
...
const getUserKeyFromCredential = (token: string | null): string | null => {
...
export default function ReceiptHistory({ onBack }: ReceiptHistoryProps) {
  ...
  const [credential, setCredential] = useState<string | null>(() => {
    try {
      return localStorage.getItem('ed_googleCredential');
    } catch {
      return null;
    }
  });
  ...
  useEffect(() => {
    const onStorage = (e: StorageEvent) => {
      if (e.key !== 'ed_googleCredential') return;
      setCredential(e.newValue);
    };
    ...
    const intervalId = window.setInterval(() => {
      try {
        const next = localStorage.getItem('ed_googleCredential');
        setCredential((prev) => (prev === next ? prev : next));
      } catch {
        // ignore
      }
    }, 750);
  ...
  const userKey = useMemo(() => getUserKeyFromCredential(credential), [credential]);
  ...
  }, [supabase, userEmail, userKey, receiptsVersion]);
